...
export const AnswerBox = ({ answer }: Props) => {
	...
	return (
		<div className="rounded-[4px] w-full p-0 grid grid-cols-4 items-center gap-0.5">
			...
			<div className={'answerBoxBorder flex flex-col items-center justify-center' + (answer.isStationTheSame ? ' correct' : answer.stationCharStatus.includes(true) ? ' halfcorrect' : '')}>
				<p className="text-center text-[14px] font-extrabold">
					<AnswerBoxText text={station.name + '駅'} status={answer.stationCharStatus} />
				</p>
			</div>
			<div className={'answerBoxBorder flex flex-col items-center justify-center'} style={{ borderColor: getDistanceBorderColor(answer.distanceKm) }}>
